// WKV-4 CUDA Kernel — Adapted from BlinkDL/RWKV-LM
// https://github.com/BlinkDL/RWKV-LM/blob/main/RWKV-v4/cuda/wkv_cuda.cu
//
// Compile on Kaggle:
//   nvcc -O3 -shared -Xcompiler -fPIC -DTmax=256 --maxrregcount 60 \
//     -o libwkv_cuda.so cuda/wkv_cuda.cu
//
// Each thread handles one (batch, channel) pair and scans T timesteps.
// Uses max-tracking for numerical stability.
...
__global__ void kernel_forward(const int B, const int T, const int C,
    ...
    // p, q = running sums divided by exp(o)
...
__global__ void kernel_backward(const int B, const int T, const int C,
    ...
    // Local arrays for intermediate values
    ...
    // Forward sweep (recompute + gather info for backward)
    for (int i = 0; i < T; i++) {
        const int ii = i * C;
        F no = max(o, k[ii] + u);
        F A = exp(o - no);
    ...
    // Backward sweep (reverse pass for gk, gv)
    F gp = 0, gq = 0;
    o = MIN_VALUE;
    for (int i = T - 1; i >= 0; i--) {
        const int ii = i * C;
    ...
    // Accumulate per-batch gw, gu
    // NOTE: gw is multiplied by w because of the -exp(w) preprocessing
...
void wkv_forward(int B, int T, int C,
...
// Allocate GPU memory
void* wkv_cuda_malloc(size_t size) {
    void* ptr = NULL;
    cudaMalloc(&ptr, size);
    return ptr;
}
...
// Free GPU memory
void wkv_cuda_free(void* ptr) {
    cudaFree(ptr);
}

// Copy host → device
void wkv_cuda_copy_h2d(void* dst, const void* src, size_t size) {
    cudaMemcpy(dst, src, size, cudaMemcpyHostToDevice);
}

// Copy device → host
void wkv_cuda_copy_d2h(void* dst, const void* src, size_t size) {
    cudaMemcpy(dst, src, size, cudaMemcpyDeviceToHost);
}

// Zero GPU memory
void wkv_cuda_zero(void* ptr, size_t size) {
