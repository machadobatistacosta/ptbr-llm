//! WKV Linear Attention - Versão MÍNIMA ESTÁVEL
//! 
//! REMOVIDO: LogSumExp trick (causava OOM)
//! REMOVIDO: Tensores intermediários extras
//! MANTIDO: Matmul com decay aprendido
...
pub fn wkv_linear<B: Backend>(
    ...
    let current_num = k_exp.clone() * u_exp.clone() * v.clone();
    let current_den = k_exp.clone() * u_exp;
    ...
    // Cumsum via matmul
    let wv = (k_exp.clone() * v).swap_dims(1, 2).reshape([batch_size * channels, seq_len]);
    let cum_wv = decay.clone().matmul(wv.transpose()).transpose()
        .reshape([batch_size, channels, seq_len]).swap_dims(1, 2);
    
    let kexp_flat = k_exp.swap_dims(1, 2).reshape([batch_size * channels, seq_len]);
    let cum_kexp = decay.matmul(kexp_flat.transpose()).transpose()
        .reshape([batch_size, channels, seq_len]).swap_dims(1, 2);
    
    // Output
    let num = cum_wv + current_num;
    let den = cum_kexp + current_den + 1e-6;
    (num / den).clamp(-50.0, 50.0)
}
